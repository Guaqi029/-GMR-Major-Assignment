...
class GeneralMotionRetargeting:
    ...
    def __init__(
        ...
        phase_weight_scales: dict | None = None,
    ) -> None:
        ...
        self.phase_weight_scales = phase_weight_scales or {
            "contact": {
                "pelvis": (1.15, 1.15),
                "spine3": (1.2, 1.4),
                "left_shoulder": (1.3, 1.2),
                "right_shoulder": (1.3, 1.2),
                "left_elbow": (1.35, 1.25),
                "right_elbow": (1.35, 1.25),
                "left_wrist": (1.6, 1.3),
                "right_wrist": (1.6, 1.3),
            }
        }
        ...
        self.task_errors1 = {}
        self.task_errors2 = {}
        self.task_base_costs1 = {}
        self.task_base_costs2 = {}
    ...
    def setup_retarget_configuration(self):
        ...
        for frame_name, entry in self.ik_match_table1.items():
            ...
            if pos_weight != 0 or rot_weight != 0:
                ...
                self.task_base_costs1[task] = (float(pos_weight), float(rot_weight))
        ...
        for frame_name, entry in self.ik_match_table2.items():
            ...
            if pos_weight != 0 or rot_weight != 0:
                ...
                self.task_base_costs2[task] = (float(pos_weight), float(rot_weight))
                self.tasks2.append(task)
                self.task_errors2[task] = []

    def set_phase(self, phase_name=None):
        self.current_phase = phase_name or "default"

    def _apply_phase_costs(self):
        if not self.phase_aware:
            phase_scale = {}
        else:
            phase_scale = self.phase_weight_scales.get(self.current_phase, {})

        for body_name, task in self.human_body_to_task1.items():
            base_pos, base_rot = self.task_base_costs1[task]
            pos_scale, rot_scale = phase_scale.get(body_name, (1.0, 1.0))
            setattr(task, "position_cost", base_pos * pos_scale)
            setattr(task, "orientation_cost", base_rot * rot_scale)

        for body_name, task in self.human_body_to_task2.items():
            base_pos, base_rot = self.task_base_costs2[task]
            pos_scale, rot_scale = phase_scale.get(body_name, (1.0, 1.0))
            setattr(task, "position_cost", base_pos * pos_scale)
            setattr(task, "orientation_cost", base_rot * rot_scale)
